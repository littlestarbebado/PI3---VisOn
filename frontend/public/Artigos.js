...
export default function Artigos() {
  ...
  // Carregar artigos publicados da API
  useEffect(() => {
    api.get('/artigos')
  ...
  // Primeiro artigo é o destaque, o resto vai para a grelha
    ...
    <>
      ...
      <section className="section-white">
        <div className="container">
          ...
          {carregando && (
            <p style={{ color: '#64748b' }}>A carregar artigos...</p>
          )}

          {!carregando && artigos.length === 0 && (
            <p style={{ color: '#64748b' }}>Ainda não há artigos publicados.</p>
          )}
          ...
          {!carregando && destaque && (
            <div className="artigo-destaque mb-5">
              <img
                src={destaque.imagem || 'https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=600&q=80'}
                alt={destaque.titulo}
              ...
              <div className="artigo-destaque-body">
                <div className="d-flex align-items-center gap-2 mb-2" style={{ flexWrap: 'wrap' }}>
                  <span style={{ color: '#64748b', fontSize: '0.8rem' }}>
                    ...
                    {destaque.dataPublicacao?.slice(0, 10)}
                  </span>
                  <span style={{ color: '#64748b', fontSize: '0.8rem' }}>
                ...
                <h2 style={{ fontFamily: 'var(--font-display)', fontWeight: 800, fontSize: '1.5rem', color: '#0d1117', marginBottom: '0.75rem' }}>
                  {destaque.titulo}
                </h2>
                <p style={{ color: '#64748b', marginBottom: '1.2rem', fontSize: '0.92rem' }}>
                  {destaque.resumo}
                </p>
                <Link to={`/artigos/${destaque.slug}`} className="btn-vison-dark" style={{ textDecoration: 'none', fontSize: '0.85rem' }}>
                  Ler Artigo Completo
                </Link>
              </div>
            </div>
          ...
          {!carregando && resto.length > 0 && (
            <>
              <h4 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: '#0d1117', marginBottom: '1.5rem' }}>
                Últimas Publicações
              </h4>
                ...
                {resto.map(a => (
                  <div key={a.id} className="col-md-4">
                    <div className="artigo-card">
                      <img
                        src={a.imagem || 'https://images.unsplash.com/photo-1563986768609-322da13575f3?w=600&q=80'}
                        alt={a.titulo}
                      />
                      <div className="artigo-card-body">
                        ...
                        <h6 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: '#0d1117', marginBottom: '0.4rem', fontSize: '0.95rem' }}>
                          {a.titulo}
                        </h6>
                        <p style={{ color: '#64748b', fontSize: '0.83rem', marginBottom: '0.8rem' }}>
                          {a.resumo}
                        </p>
                        <div className="d-flex align-items-center justify-content-between">
                          <span style={{ color: '#94a3b8', fontSize: '0.78rem' }}>
                            <i className="bi bi-person me-1" />{a.autor || 'Redação'}
                          </span>
                          <Link to={`/artigos/${a.slug}`} style={{ color: '#3b82f6', fontSize: '0.83rem', fontWeight: 600, textDecoration: 'none' }}>
                            Ler mais
                          </Link>
                        </div>
                      </div>
          ...
          <div className="mb-5">
            <h4 style={{ fontFamily: 'var(--font-display)', fontWeight: 700, color: '#0d1117', marginBottom: '1rem' }}>
              Categorias
            </h4>
            <div className="d-flex flex-wrap gap-2">
              <button
                onClick={() => setCatAtiva('')}
                style={{
                  background: catAtiva === '' ? '#0d1117' : '#f1f5f9',
                  color: catAtiva === '' ? '#fff' : '#475569',
              ...
              {CATEGORIAS.map(c => (
                <button
                  ...
                  style={{
                    background: catAtiva === c ? '#0d1117' : '#f1f5f9',
                    color: catAtiva === c ? '#fff' : '#475569',
                    border: 'none', borderRadius: 20,
                    padding: '0.35rem 1rem',
                    fontSize: '0.83rem', fontWeight: 600, cursor: 'pointer'
                  }}
          ...
          <div style={{ background: '#0d1117', borderRadius: 16, padding: '2.5rem', textAlign: 'center', border: '1px solid rgba(255,255,255,0.08)' }}>
            ...
            <p style={{ color: 'var(--vison-gray)', fontSize: '0.9rem', marginBottom: '1.2rem' }}>
              Receba as últimas notícias e artigos diretamente no seu email
            </p>
            {subOk ? (
              <p style={{ color: '#10b981', fontWeight: 600 }}>
                <i className="bi bi-check-circle me-2" />Subscrito com sucesso!
              </p>
            ) : (
              <div className="d-flex justify-content-center gap-2" style={{ maxWidth: 400, margin: '0 auto' }}>
                ...
                />
                <button onClick={() => { if (email) setSubOk(true); }} className="btn-vison-primary">
                  Subscrever
                </button>
              </div>
            )}
